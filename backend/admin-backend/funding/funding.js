      ...
      RETURNING *`,
       ...
       VALUES ($1, $2, 'Admin', $3, $4)`,
        ...
        if (userId && finalStatus === 'Offer Under Review') {
            ...
            await createNotification(userId, 'funding_approval', message, { fundingId: request.id, projectId: request.project_id }, `/v1/auth/profile?tab=funding`);
        }
...
export const founderRespondToFunding = async (fundingId, userId, action) => {
    try {
        ...
        if (projectOwnerId !== userId) throw new Error("Unauthorized: You do not own this project.");
