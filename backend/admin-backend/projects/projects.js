...
// Update Project Approval Status with Custom Message / Rejection Reason
export const updateProjectApproval = async (id, approved, message) => {
  // Check existing state first to ensure single-decision rule
  ...
  const existingApproved = existing.approved === true || String(existing.status).toLowerCase() === "approved";
  const existingRejected = String(existing.status).toLowerCase() === "rejected";

  if (existingApproved || existingRejected) {
    const error = new Error(
      "This project has already been finalized (Approved/Rejected) and cannot be changed."
    );
  ...
  const res = await pool.query(
    `UPDATE projects
     ...
     WHERE id = $3
     RETURNING *;`,
    [isApproved, statusVal, id],
  );

  const project = res.rows[0];
  if (!project) return null;

  // Find all entrepreneurs associated with this project
  const entrepreneurs = await pool.query(
    "SELECT DISTINCT user_id FROM project_entrepreneurs WHERE project_id = $1 AND user_id IS NOT NULL",
    [id]
  );

  const userIds = new Set(entrepreneurs.rows.map((r) => r.user_id));

  // Default message if not supplied
  const defaultMsg = isApproved
    ? `🎉 Congratulations! Your project "${project.name || project.title}" has been APPROVED for the DxValley Incubation Center.`
    : `❌ Notice regarding project "${project.name || project.title}": Your project submission was rejected. Reason: ${message || "Application criteria not met."}`;

  const notificationMessage = message && message.trim() ? message.trim() : defaultMsg;
  ...
  for (const uid of userIds) {
    if (uid) {
      await createNotification(
        ...
        notificationMessage,
        { projectId: project.id, approved: isApproved },
        `/v1/auth/profile?tab=projects`
      );

      eventBus.emit("project.approval_changed", {
        projectId: id,
