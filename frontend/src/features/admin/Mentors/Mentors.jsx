...
const Mentors = () => {
  ...
  const fetchTotals = useCallback(async () => {
    try {
      const response = await fetch("/api/admin/mentors/totals", {
        credentials: "include",
      });
      ...
      console.error("Failed to fetch mentor totals:", error);
    }
  }, []);
  ...
  const fetchMentors = useCallback(async (page = 1) => {
    ...
    try {
      const response = await fetch(`/api/admin/mentors?page=${page}`, { credentials: 'include' });
      const data = await response.json();
      const newMentors = Array.isArray(data) ? data : [];
  ...
  useEffect(() => {
    fetchMentors(1);
    fetchTotals();
  }, [fetchMentors, fetchTotals]);

  const handleLoadMore = () => {
    const nextPage = mentorsPage + 1;
    setMentorsPage(nextPage);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`/api/admin/mentors/${id}`, { method: 'DELETE', credentials: 'include' });
      setMentorsPage(1);
      fetchMentors(1);
      fetchTotals();
    } catch (err) {
      ...
      <div className="space-y-6 font-sans">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <StatCard
              ...
              value={totals.assigned_projects}
              icon={Briefcase}
            ...
            <>
              ...
              <div className="flex flex-col items-center gap-2">
                {hasMore ? (
                    ...
                    <p className="text-xs text-[#526274] font-medium">No more mentors to load</p>
                )}
              </div>
            </>
        ) : (
