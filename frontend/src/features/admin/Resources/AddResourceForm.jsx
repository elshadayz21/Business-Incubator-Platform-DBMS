...
const AddResourceForm = ({ onClose, onSuccess, initialData }) => {
  ...
  // ADDED 'status' TO THE FORM STATE!
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        status: initialData.status || "available", // LOAD STATUS WHEN EDITING
      });
    }
  }, [initialData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        const response = await fetch(`/api/admin/resources/${initialData.id}`, {
          method: 'PUT',
      ...
      <form onSubmit={handleSubmit} className="space-y-5 font-sans">
        ...
        <div>
          ...
          <select
              ...
              className="w-full px-4 py-2.5 bg-white border border-[#D6E4EA] rounded-xl text-xs font-medium text-[#111827] outline-none focus:border-[#00ADEF] capitalize cursor-pointer"
          >
