...
// A Telegram-style messenger: a contact list on the left (every person this
// user is allowed to message, most recently active first) and the selected
// conversation's history on the right, with a composer at the bottom.
// Works for every role — the backend decides who can message whom.
const ChatPanel = ({ currentUser }) => {
  ...
  const fetchMessages = useCallback(async (contactId, { silent } = {}) => {
    ...
    try {
      ...
      const data = await res.json();
      // Ignore a stale response if the user has since switched conversations.
